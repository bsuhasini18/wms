...
function headerReduce(headers, format) {
  ...
  composed["Last-Modified"] = new Date(
  ...
  if (!etag.length) {
    composed["ETag"] =
      '"' +
      crypto.createHash("md5").update(composed["Last-Modified"]).digest("hex") +
      '"';
  } else {
    composed["ETag"] =
      etag.length === 1
        ? etag[0]
        : '"' +
          crypto.createHash("md5").update(etag.join(",")).digest("hex") +
          '"';
  }
